...
// word2vec to replace bag of words
// stopwords is making the loss larger: I need to remove some of them perhaps.
// got loss: 1.24 on epoch 199 with this setup, 0.46 for 500 epochs, 0.3 for 600 epochs
// removed the labels array, it was not needed
...
function preload() {
	// load the training JSON file here so we don't need a callback:
	data = loadJSON("intents.json");

	// Create a word2vec instance:
	//wordVectors = ml5.word2vec("wordvecs10000.json", modelLoaded);
...
function setup() {
	...
	NN_initialize(unique_words.length);
...
function prepare_training_set() {
	...
	for (var i in data.intents) { // i is each intent row

		for (var j in data.intents[i].patterns) { // j is each pattern in patterns

			// remove stopwords:
			// var string = nlp(removeStopWords(data.intents[i].patterns[j])).normalize();

			// convert string to nlp object (removes punctuation also):
			var string = nlp(data.intents[i].patterns[j]).normalize();

			// get the tokenized word array (split into words and lowercase) from the pattern phrase:
			var arr = string.toLowerCase().termList(); // arr is an array of objects
	...
	//user patterns (pattern_word_list). Each vector length will be equal to the amount of unique words in 
	//the intents space. It is essentially a binary histogram of word appearances (1 if exists, 0 if not). 
	for (var phrase = 0; phrase < pattern_word_list.length; phrase ++) {
		...
		// add the words of each pattern phrase with the word2vec algorithm:
		//wordVectors.add(pattern_word_list[phrase], function(err, results) {
  		//	console.log(results);
		//});
...
function NN_initialize(input_size) {
	...
	let options = {
		...
		outputs: data.intents.length, // how many different outputs
		task: 'classification',
		debug: true
	}
	// initialize the NN:
...
function proccess_phrase(phrase) {
	...
	var string = nlp(phrase).normalize();

	// get the tokenized word array (split into words) from the pattern phrase:
	var arr = string.toLowerCase().termList(); // pattern is an array of strings
...
function gotResults(error, results) {
	...
	// check how clear is the understanding: if difference in confidence between 0 and 1 are more than 0.20
	if ( results[0].confidence - results[1].confidence > 0.1 ) {
		// get the most probable intent tag (index 0) from the result:
		var tag = results[0].label;
	} else {
		tag = "undefined";
	...
	// for each intent:
	for (var i in data.intents) {
		if (data.intents[i].tag === tag) {
			// get a random number or response from the list:
			var k = Math.round(Math.random(data.intents[i].responses.length));
			answer = data.intents[i].responses[k];
			console.log(answer);
		}
	}

	// display the response in the text field:
	bot_response.value(answer);
}

// idea: use word2vec to calculate cosine similarity between words in each response phrase, 
// and choose the one most similar to the input phrase!

// check confidence levels and if low, provide default answer
